// Tempi Cottura - v7.4.1 (core, rebuilt)
/* ====== Config ====== */
const DISHES = [
...
shareBtn.addEventListener('click', async () => {
  ...
  try {
    if (isSecure && navigator.share) {
      await navigator.share({ text: `${intro}\\n${url}` });
      return;
    }
    throw new Error('Web Share non disponibile');
  } catch(_) {
    try {
      await navigator.clipboard.writeText(`${intro} ${url}`);
...
/* ====== Hash init (only here audio prompt) ====== */
function initFromHash() {
  ...
  if (needsAudioUnlock()) injectAudioPrompt();
}
