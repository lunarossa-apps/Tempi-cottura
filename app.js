...
function initAudio(){
  if (!audioCtx) {
    ...
    audioCtx.resume(); localStorage.setItem(AUDIO_OK_KEY, '1');
  }
}
...
function showAudioPrompt(){ document.getElementById('audioPrompt')?.classList.remove('hidden'); }
function hideAudioPrompt(){ document.getElementById('audioPrompt')?.classList.add('hidden'); }
document.getElementById('enableAudio')?.addEventListener('click', ()=>{ try{ initAudio(); }catch(_){ } hideAudioPrompt(); });

/* ====== Base64URL compact share ====== */
